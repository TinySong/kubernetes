...
PullImage=$(cat <<EOF
  ...
  docker pull \${1}/\${2}/node:${CALICO_VERSION}
  docker pull \${1}/\${2}/cni:${CALICO_VERSION}
  if [ \${3} == "master" ]; then
...
docker run --net=host -d --cpu-period=100000 --cpu-quota=100000 --memory=100000000 --restart=always -v /tmp:/tmp -v /etc/machine-id:/etc/machine-id:ro -v /etc/kubernetes:/etc/kubernetes -v /etc/hosts:/etc/hosts -v /etc/resolv.conf:/etc/resolv.conf --name agent  ${REGISTRY_SERVER}/${REGISTRY_USER}/agent:${AGENT_VERSION} ${ADVERTISE_ADDRESSES_AGENT} --role=master --etcd-servers=${etcdServers} --dns-enable=true  --ssl-enable=false >/dev/null
result=\$?
if [[ \${result} -eq 0  ]];then
   echo "Kubernetes Enterprise Edition cluster deployed successfully"
else
